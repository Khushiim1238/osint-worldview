...
export function isFeatureAvailable(featureId: RuntimeFeatureId): boolean {
  // Hide V2 features for simplified presentation (under construction)
  const disabledFeatures = new Set<RuntimeFeatureId>([
    'aiOllama',
    'aiJarvis',
    'aisRelay',
    'openskyRelay',
  ]);
  if (disabledFeatures.has(featureId)) return false;
